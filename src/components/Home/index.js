        ...
        <Grid container spacing={4}>
          {slideImages.map((slide, index) => (
            <Grid item xs={12} sm={6} md={4} key={index}>
              <motion.div initial="hidden" animate="visible" variants={cardVariants} transition={{ duration: 1, delay: index * 0.2 }}>
                <Card sx={{ height: "100%", display: "flex", flexDirection: "column", backgroundColor: "rgba(255, 255, 255, 0.1)" }}>
                        ...
                        {slide.items.map((item, idx) => (
                          <li key={idx}>
                            {slide.links && slide.links[idx] ? (
                              <Link to={slide.links[idx]} style={{ textDecoration: "none", color: "inherit" }}>
